Fresh Test Observatory is a test explorer for Fresh. It discovers tests, runs them at workspace, file, selected, or cursor scope, keeps complete runner output available, navigates failures, and marks source coverage.

Install it with Package: Install from URL using https://github.com/willibrandon/fresh-test-observatory, then run Test Observatory: Open from the command palette. The package and user-facing name are intentionally the same so commands, settings, and adapter registration use one identity.
