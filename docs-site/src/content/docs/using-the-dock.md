...
Select a row to see its latest result under the tree: status, duration, source location, message, and stack trace. Press `Enter` to open the test in its file, or the failure location when the last run failed.
